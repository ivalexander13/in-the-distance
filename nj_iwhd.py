...
class IWHD:
    ...
    def _ewhd_given_h(
        self, mut_rate: float, collision_rate: float, height: float, time: float
    ):
        t = time
        r = mut_rate
        q = collision_rate
...
class InverseNJSolver(solver.NeighborJoiningSolver):
    ...
    def _spline_qdist(self, numstates: int) -> List:
        ...
        """
        state_priors = pickle.load(
            open(
                "/data/yosef2/users/richardz/projects/notebooks/full_indel_dist.pkl",
                "rb",
            )
        )
        tck = interpolate.splrep(
            list(state_priors.keys()), list(state_priors.values())
        )
        bins = []
        ...
        for i in range(len(bins) - 1):
            vals.append(
                np.mean(
                    interpolate.splev(list(range(bins[i], bins[i + 1])), tck)
                )
            )
    ...
    def set_numstates(self, numstates: int) -> None:
        """Set the number of states for the inverse weighted hamming distance
        function.
    ...
    def set_mut_prop(self, mut_prop: float) -> None:
        """Set the mutation proportion for the inverse weighted hamming
        distance function.
    ...
    def set_total_time(self, total_time: float) -> None:
        """Set the total time of a tree for the inverse weighted hamming
        distance function.
    ...
    def set_state_distribution(self, state_distribution: List) -> None:
        """Set the state distribution for the inverse weighted hamming
        distance function.
    ...
    def get_dissimilarity_map(
        ...
        """Get the dissimilarity map for the given tree. This method
        overrides the same method of the super class."""
        ...
        if hasattr(self, "mut_prop"):
            ...
            mut_prop = np.count_nonzero(cm.replace(-1, 0)) / np.count_nonzero(
                cm + 1
            )
