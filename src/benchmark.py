...
def get_stressor_by_params(lt_parameters) -> Tuple[str, str]:
    """Returns the stressor name and its value, given a list of LT Params."""
    ...
    if lt_parameters.numstates != numstates__default:
        ...
        if lt_parameters.drop_total == 0:
            ...
            stressor, param = "stoch_high_drop", int(
                lt_parameters.drop_total * 100
            )
...
# cached
def get_gt_tree(tree_parameters, lt_parameters) -> CassiopeiaTree:
    """Returns the ground truth tree given the tree and LT parameters. Currently
    defaults to loading pre-simulated trees from Richard."""
...
def get_solver_by_name(
    solver_name, priors_type="no_priors"
) -> solver.CassiopeiaSolver:  # type: ignore
    """Returns the solver given the solver name and priors type. Add a solver
    ...
    if solver_name == "greedy":
        ...
        if priors_type is not "no_priors":
            tree_solver = solver.ILPSolver(
                maximum_potential_graph_lca_distance=20, weighted=True
            )
        else:
            tree_solver = solver.ILPSolver(
                maximum_potential_graph_lca_distance=20, weighted=False
            )
    elif solver_name == "greedy_over_maxcut":
        greedy_solver = solver.VanillaGreedySolver()
        maxcut_solver = solver.MaxCutSolver()
        tree_solver = solver.HybridSolver(
            greedy_solver, maxcut_solver, lca_cutoff=20, threads=10
        )
    elif solver_name == "greedy_over_ilp":
        greedy_solver = solver.VanillaGreedySolver()
        if priors_type is not "no_priors":
            ilp_solver = solver.ILPSolver(
                maximum_potential_graph_lca_distance=20, weighted=True
            )
        else:
            ilp_solver = solver.ILPSolver(
        ...
        tree_solver = solver.HybridSolver(
            greedy_solver, ilp_solver, lca_cutoff=20, threads=10
        )
...
@cached()
def run_solver(
    tree_parameters, lt_parameters, solver_parameters
) -> CassiopeiaTree:
    """Runs the solver on a given ground_truth tree's character matrix and
    returns the resulting reconstructed tree"""
    solver = get_solver_by_name(
        solver_parameters.solver_name, priors_type=solver_parameters.priors_type
    )

    gt_tree = get_gt_tree(tree_parameters, lt_parameters)
    ...
    recon_tree = CassiopeiaTree(
        character_matrix=gt_tree.character_matrix, missing_state_indicator=-1
    )  # todo: implement priors
    solver.solve(
        recon_tree,
        collapse_mutationless_edges=solver_parameters.collapse_mutationless_edges,
    )
...
def get_score(
    ...
    """Given the parameters, get both the ground truth tree and the
    reconstructed tree, and calculate the rf or triplets correct score
    between them."""
    # get gt tree
    gt_tree = get_gt_tree(tree_parameters, lt_parameters)

    # get solved tree
    recon_tree = run_solver(tree_parameters, lt_parameters, solver_parameters)
    ...
    if metric_name == "rf":
        ...
        triplets = cas.critique.compare.triplets_correct(
            gt_tree, recon_tree, number_of_trials=500
        )
